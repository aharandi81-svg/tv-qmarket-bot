...
def main():
    ...
    KEYWORD_MACRO = [
        ...
        (["ماست", "پنیر", "شیر"], {"protein": 0.35, "fat": 0.45, "carb": 0.2}),
        (["مرغ", "گوشت", "ماهی", "میگو", "تخم مرغ", "بوقلمون", "گوساله", "بره"],
         {"protein": 1.0}),
        (["برنج", "نان", "پاستا", "ماکارونی", "سیب زمینی", "شکر", "آرد", "نشاسته",
    ...
    lines.append("")
    lines.append(
        "**اصلاح دستی ماکرو (macro overrides):** برای غذاهایی که کارت رسپی‌شان "
        "ناقص بود (مثلاً «چلو کباب کوبیده پرسنلی» فقط مواد برنج را ثبت کرده و هیچ "
        "قلمی برای گوشت کوبیده ندارد) یا نوع غذا (پیتزا، سالاد، دسر، نوشیدنی) با "
        "محاسبه خام وزنی در تضاد بود، ماکرو به‌صورت دستی روی مقادیر واقع‌بینانه "
        "بازنویسی شد (فهرست کامل در بالا). علاوه بر override های دستی، یک لایه "
        "sanity-check سطح-دسته هم اجرا می‌شود: اگر غذایی در دسته «دسر» باشد ولی "
        "پروتئین/سبزی محاسبه‌شده بیش از حد باشد، یا «پیتزا» باشد ولی کربوهیدرات "
        "محاسبه‌شده خیلی پایین بیاید، یا «سالاد» باشد ولی سهم سبزی محاسبه‌شده کم "
        "باشد، به یک مقدار پیش‌فرض معقول برای آن دسته برگردانده می‌شود."
    )
    lines.append("")
    lines.append(
